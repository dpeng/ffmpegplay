...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
        ...
        height = is->height;
        width = ((int)floor(height * aspect_ratio)) & ~1;
        if (width > is->width) {
            width = is->width;
            height = ((int)floor(width / aspect_ratio)) & ~1;
        }
        x = (is->width - width) / 2;
...
static void toggle_audio_display(void)
{
    if (cur_stream) {
        ...
        cur_stream->show_mode = (ShowMode)((cur_stream->show_mode + 1) % SHOW_MODE_NB);
        fill_rectangle(screen,
...
static const OptionDef options[] = {
	{	},
	/*
    { "x", HAS_ARG | OPT_FUNC2, {(const char*)opt_width}, "force displayed width", "width" },
